...
catch {set coef_files [glob -nocomplain LowPass_40Hz_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VERILOG"
set ipfs_ext vo
if {[file exists LowPass_40Hz.vo]} {
	set hdl_ext v
} else {
	puts "Warning: Could not find LowPass_40Hz.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_LowPass_40Hz -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE LowPass_40Hz.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE LowPass_40Hz_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_LowPass_40Hz.vhd -section_id tb -library work
