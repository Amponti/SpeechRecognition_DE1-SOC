...
void calc_FIR_coefs(FIR_filter * filter)
{
	...
	for(i=0;i<N;i++)
	{
		...
		fprintf(pFile,"%f, ",filter->coefs[i]);
		//filter->taps[i]=0;
		n = n+1;
	}
	fclose(pFile);
...
void save_signal( SIGNAL * FIR_signal, char * filename)
{
	...
	swrite = fwrite(&riff[0], 1, 4, pFilt);

	swrite = fwrite(&fsize, 1, 4, pFilt);

	swrite = fwrite(&wave[0], 1, 4, pFilt);

	swrite = fwrite(&fmt[0], 1, 4, pFilt);

	swrite = fwrite(&nbytes, 1, 4, pFilt);

	swrite = fwrite(&ccode, 1, 2, pFilt);
	nbytes = nbytes-2;

	swrite = fwrite(&channels, 1, 2, pFilt);
	nbytes = nbytes-2;

	swrite = fwrite(&rate, 1, 4, pFilt);
	nbytes = nbytes-4;

	swrite = fwrite(&avgrate, 1, 4, pFilt);
	nbytes = nbytes-4;

	swrite = fwrite(&blockalign, 1, 2, pFilt);
	nbytes = nbytes-2;

	swrite = fwrite(&bps, 1, 2, pFilt);
	nbytes = nbytes-2;

	swrite = fwrite(&data[0], 1, 4, pFilt);

	swrite = fwrite(&csize, 1, 4, pFilt);
